// Tesla Stock Watch — Primary scraper (puppeteer-extra + stealth plugin)
// Launches headed Chrome with a persistent profile to bypass Akamai CDN.
// Calls Tesla's inventory API from within the page context.
...
async function clearChromeProfileProcesses(reason) {
  ...
  console.warn(`[scraper:primary] Clearing ${pids.length} Chrome process(es) for ${CHROME_USER_DATA_DIR}: ${reason}`);
  ...
  console.warn(`[scraper:primary] Force killing ${pids.length} Chrome process(es) still holding ${CHROME_USER_DATA_DIR}`);
  for (const pid of pids) {
...
async function minimizeBrowserWindow(page) {
  ...
  try {
    ...
    console.warn(`[scraper:primary] Could not minimize Chrome window: ${e.message}`);
  }
}
...
async function ensureBrowser() {
  if (_browser && browserConnected(_browser) && Date.now() - _browserAt < BROWSER_MAX_AGE) {
    ...
    console.warn(`[scraper:primary] Recycling Chrome: ${recycleReason}`);
  }
  if (_browser) {
  ...
  _installing = true;
  try {
    console.log("[scraper:primary] Launching headed Chrome…");
    ...
    _browserAt = Date.now();
    console.log(`[scraper:primary] Chrome launched (${CHROME.source}: ${CHROME.path})`);
...
async function configurePageIdentity(page) {
  ...
  await page.emulateTimezone(BROWSER_TIMEZONE).catch((e) => {
    console.warn(`[scraper:primary] Could not set timezone ${BROWSER_TIMEZONE}: ${e.message}`);
  });
  if (PROXY_FAILOVER_ENABLED && CHROME_PROXY_USERNAME && CHROME_PROXY_PASSWORD) {
    await page.authenticate({
      ...
      console.warn(`[scraper:primary] Could not set proxy credentials: ${e.message}`);
    });
  }
}
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (/access denied|blocked/i.test(title)) {
        blocked = true;
        console.warn(`[scraper:primary] Blocked on attempt ${i + 1}, waiting…`);
        await sleep(10000);
        continue;
      }
      ...
      console.warn(`[scraper:primary] Warmup error (${i + 1}): ${err.message}`);
...
async function visitSecondaryPage(page) {
  ...
  _secondaryPageIndex++;
  console.log(`[scraper:primary] Visiting secondary page: ${url}`);
...
async function scrapeInventory(maxPages = DEFAULT_MAX_PAGES) {
  ...
  try {
    ...
    if (!warmed.ok) {
      throw new Error(teslaMetaMessage("Page blocked by Akamai after retries", {
        phase: "warmup_page",
        requestCount,
    ...
    if (!cookieState.sessionHealthy) {
      console.warn(`[scraper:primary] Suspicious cookie state after warmup: ${cookieState.totalCookies} total, ${cookieState.akamaiCookieCount} Akamai cookies`);
    }
    ...
    console.log(`[scraper:primary] ${total} total vehicles in AU inventory`);

    const firstResults = firstJson.results ?? [];
    if (Array.isArray(firstResults) && firstResults.length) {
      allResults.push(...firstResults);
      console.log(`[scraper:primary] Page 1: +${firstResults.length} vehicles (${allResults.length}/${total})`);
    }
    ...
    while (allResults.length < total && pagesFetched < safeMaxPages) {
      ...
      try {
        ...
        if (/HTTP 403|akamai|access denied|blocked/i.test(String(e.message || ""))) {
          ...
          console.warn(`[scraper:primary] Mid-pagination block after ${pagesFetched} page(s); returning partial data`);
          break;
        }
        throw e;
      }
      ...
      console.log(
        `[scraper:primary] Page ${Math.floor(offset / PAGE_SIZE) + 1}: +${results.length} vehicles (${allResults.length}/${total})`
      );
      offset += PAGE_SIZE;
    }
    ...
    if (capped) {
      console.warn(`[scraper:primary] Pagination capped at ${safeMaxPages} page(s): ${allResults.length}/${total}`);
    }
...
async function destroyBrowser() {
  if (_browser) {
    ...
    console.log("[scraper:primary] Browser closed");
  } else if (chromeProfilePids().length) {
